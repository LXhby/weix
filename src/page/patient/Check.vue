<template>
    ...
    <div class="tab-content" v-show="tabitem === '上传材料'">
      <div v-for="item in checkItems" :key="item.id">
    ...
    <div class="page-ft" v-transfer-dom>
      <x-button type="warn" :disabled="submitBtnDisabled" @click.native="submitForm()" >下一步</x-button>
    </div>
	</div>
</template>
...
export default {
  ...
  methods: {
    ...
    initData(){
      ...
      this.$http.post('pap/get-checkitems',{'patient_id':this.patient_id,'category':global.CATEGORY_ECONOMIC})
        .then(response=>{
  ...
  computed: {
    redirectUrl: function(){
      return { name: 'patient-medical', params: { 'patient_id': this.patient_id } };
    }
  },
  data () {
    return {
