<template>
  <div class="member-center">
    ...
    <group class="myform" label-margin-right="2em" label-width="7.5em" title="请按顺序选择省市、医院、医生、药房">
      <x-address :required="true" v-validate="'required'"  title="项目所属省市" v-model="formdata.place" placeholder="请选择省市" value-text-align="left" raw-value :list="addressData" hide-district @on-shadow-change="getName" style="white-space: nowrap;"></x-address>
    </group>
    ...
    <div v-transfer-dom>
      <popup v-model="showDrugstore"  height="250px">
...
export default {
  ...
  methods: {
    initData(){
      this.$http.get('patients',{params:{'PatientSearch[id]':this.$route.params.id}})
        .then(response=>{
          //初始化用户信息
