  ...
  <div class="member-center">
    <div v-show="step">
        <group title="申请注册" class="mylogin" label-margin-right="2em" label-width="5em">
          <x-input :required="true"  name="name"  v-model="formdata.name" title="姓名" placeholder="请输入姓名" ></x-input>
  ...
  methods: {
                  ...
                  switch(response.data.status)
                  {
                    ...
                    case(global.CHECK_IN_WECHAT):
                      ...
                      if(response.data.status=='GO_PASSWORD'){
                        ...
                        this.$vux.alert.show('您已成功注册，请登录使用')
    ...
    submitForm(){
      if(!this.formdata.password ||  !this.formdata.password2){
        ...
        this.$http.post('patient/register',this.formdata)
          .then(response=>{
            if(response.data==true){
              if(this.flag){
                this.$router.push({name:'register-apply'})
              }else{
                this.$vux.alert.show('注册成功！请登录使用')
                this.$router.push({name:'login-password'})
              } 
            }else{
